  ...
  <div id="musicPlayer">
        ...
        <div class="music_list">
          <div class="list_l">
            ...
            <ul class="_list scrollbar">
              <li v-for="(item, index) in thisMusicList" :key="index" @mouseover="ButtonActive(index)" @dblclick="ListPlay((thisListPage - 1) * 50 + index)">
                ...
                <transition name="list_button">
                  <div class="music_button" v-if="listButtonActiveIndex == index">
                    <div class="list_play" title="播放这首歌" :style="{ backgroundImage: 'url(' + listPlay + ')' }" @click="ListPlay((thisListPage - 1) * 50 + index)"></div>
                    <div class="list_play" title="添加到收藏歌单" :style="{ backgroundImage: 'url(' + add + ')' }" @click="ListAdd(item)" v-if="thisMusicType != -1"></div>
                  </div>
                </transition>
              </li>
            </ul>
            <div class="list_page">
              ...
              <div class="page_next" v-show="thisListPage != Math.ceil(musicList.length / 50)" @click="ListChange(false)">下一页</div>
            </div>
          </div>
          <div class="list_r">
    ...
    <div class="bbox" :class="{ bbox_active: disActive }">
      ...
      <div class="box" :style="{ backgroundImage: 'url(' + musicImg + ')' }" :class="{ box_active: disActive }" @dblclick="DisList">
        ...
        <div class="music_dis">
          <div class="dis_list" @click="DisList">更多</div>
...
export default {
  ...
  methods: {
    ...
    _getInfo() {
      getMusicUrl(this.musicList[this.thisMusicIndex].id)
        .then(res => {
